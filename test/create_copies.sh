...
for ((i=1; i<=$MULTIPLIER_COUNT; i++)); do
  echo "Running iteration $i of $MULTIPLIER_COUNT"
    cat counts.txt | xargs -IXXX bash -c "bash copy_files.sh --no-summary --source-file=json/incidents.json --output-dir=/tmp/input --name=\$(date +%s.%N)-\$(cat /dev/urandom | tr -dc A-Za-z0-9 | head -c\${1:-5}).json  --count=XXX"
done
