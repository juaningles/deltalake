...
def lazycount(ldf):
    return ldf.select(pl.len()).collect().row(0)[0]
...
df_2 = ( pl.scan_ndjson(f'{input_files}'))
...
if storage_options:
    print("Storage Options: ")
    print(storage_options)
df_2.sink_parquet(f"{output_path}/{entity_name}")
...
if entity_name:
    # Read the Delta Lake table
    dt = pl.read_parquet(f"{output_path}/{entity_name}")
